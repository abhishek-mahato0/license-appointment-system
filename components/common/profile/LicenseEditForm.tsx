...
import { citizenshipData, licenseData } from "@/components/detailform/FormData";
import { handleFormData } from "../FormDetail/HandleFormData";
import FullFlex from "../Fullflex";
import { Button } from "@/components/ui/button";

export default function LicenseEditForm({ licenseInformation }: any) {
  const closeref = useRef<any>(null);
  const { toast } = useToast();
  const userInfo = JSON.parse(localStorage.getItem("userInfo") || "{}");
  const router = useRouter();
  const { register, handleSubmit } = useForm({
    defaultValues: licenseInformation,
  });
  const handlePersonalData = async (data: any) => {
    try {
      const payload = {
        ...data,
        front: licenseInformation.front,
        back: licenseInformation.back,
      };
      if (!userInfo?.id) {
        toast({
          title: "Error",
      ...
      try {
        const res = await apiinstance.put(
          `/user/information/${userInfo?.id}/license`,
          payload
        );
        if (res.status != 200) {
          return toast({
    ...
    <Dialog>
      ...
      <DialogContent className=" min-w-fit">
        <DialogHeader>
          ...
          <DialogDescription>
            This is the license information edit form.
          </DialogDescription>
        </DialogHeader>
